...
- Pocket, LCD, Cartridge, and CRT are the layered theme pilot. Their packages place the car first and lane marks second in the foreground Subject group, with the road in the World group behind both. Pocket preserves the approved three-path interrupted-center composition; LCD, Cartridge, and CRT preserve four broad boundary paths. Every path and both road edges derive from the icon's shared vanishing point, extend from the top to the bottom edge, and grow in width, length, and spacing toward the viewer. The outer paths project beyond the lower corners and the inner paths remain visible around or below the car. Canonical iPad SpriteKit cars remain transparent, nearest-neighbour, opaque, glass-free layers.
...
- CRT Default uses green `#087D17` off-road, deep-navy `#151D29` road, and yellow `#FFD11A` lane marks. CRT Dark uses green-grey `#3A413B` off-road and charcoal-navy `#10141B` road while retaining yellow `#F6C928` marks. The lighter off-road establishes the track silhouette before the frontmost vignette darkens the edges. Its canonical 16-bit iPad car and four-path geometry remain identical across appearances. All four path centers remain inside the projected road polygon at every depth; the outer marks sit slightly inboard rather than straddling the off-road boundary. A separate full-canvas `Accents` group sits in front of Subject and World and supplies the CRT scanline/highlight pattern plus radial vignette. The overlay has no glass, shadow, or translucency; Dark reduces scanline opacity from `0.18` to `0.14` but strengthens the edge-only vignette from `0.26` to `0.30`, beginning at `58%` radius, so the curved-tube effect returns without obscuring central car or road detail.
- Mono annotations establish one shared hierarchy for Clear Light/Dark and Tinted Light/Dark across every layered theme package: the canonical car remains at `1.0` opacity, lane marks use `0.75`, and the road uses `0.45`. CRT retains its frontmost scanline and vignette overlay at `0.60`. Default and Dark keep every artwork layer at full opacity. Use flat `opacity-specializations`; do not bake this hierarchy into replacement artwork.
...
- Disc and Polygon remain flattened during this pilot. Classic is unchanged.
- Gallery previews are `512×512` ordinary image assets. Each pilot imageset contains a Default and a luminosity-Dark rendition generated from the corresponding package sources; Xcode derives device density renditions at build time.
- `./retrorapid assets app-icons` deterministically regenerates the Pocket/LCD/Cartridge/CRT SVG and car layers, copies both approved Special Edition Default/Dark companion sources into their packages, and generates the six adaptive gallery preview pairs. `--dry-run` reports the mutation plan; `--check` rejects generated-output drift. Icon Composer appearance/material configuration remains in each tracked `icon.json` and is never overwritten by the generator.
