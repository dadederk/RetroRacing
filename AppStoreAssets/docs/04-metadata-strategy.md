...
| Territory | Primary locale | Also indexes (relevant to us) | Keyword strategy |
|---|---|---|---|
| **United States** | English (US) | **Spanish (Mexico)** among others | Core English discovery in **en-US** (`retro`, `offline`, `voiceover`, `haptics`, `leaderboard`). Mexico Spanish in **es-MX** without repeating en-US tokens. |
| **United Kingdom** | English (UK) | English (US) among others | Complementary terms in **en-GB** (`endless`, `highway`, `skill`, `nostalgia`) — no tokens already in name/subtitle (`arcade`, `racer`, `traffic`, `lanes`). |
| **Canada** | English (Canada) | English (US) among others | **en-CA** (`scoreboard`, `ipad`, `mobile`, `quick`) separate from **en-US**; no `dodge` / `lanes` / `racer`. |
| **Australia** | English (Australia) | — | **en-AU** (`mobile`, `overtake`, `quick`, `skill`) separate from **en-US** and **en-GB**; no `racing` / `dodge` / `lanes`. |
