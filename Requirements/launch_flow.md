...
- watchOS keeps its platform navigation. visionOS uses the shared universal-style menu and one shared-session coordinator across Classic and a pushed gravity-aligned volume. Play in 3D acquires transition and spatial-ready pause reasons, preflights Polygon models, and pushes the volume from Classic; renderer readiness dismisses Classic so the two presentations are never visible or playable together. The volume stays where placed, moves through the system window bar, and may snap to horizontal surfaces. Return, system volume close, backgrounding, and SharePlay reopen Classic when needed and restore the exact run. Classic gameplay keeps an X in the top-left and Pause/Resume in the top-right; Settings is available from the menu only. See [visionos_gameplay.md](visionos_gameplay.md).

## Play with Friends

- The button is shown only when the composition root supplies `onPlayWithFriendsRequest`.
- It starts a synchronized race on iOS/iPad/macOS/tvOS and visionOS Classic, and is hidden on watchOS. tvOS explains the FaceTime-group prerequisite when direct activation is not eligible. visionOS automatically dismisses the spatial volume to Classic before applying an incoming match.
