...
| Retro Cartridge | [`retro-cartridge-v4.png`](retro-cartridge-v4.png) | Full-frame cream cartridge, app-pink accents, printed RetroRapid! label, and gold connector pins. |
| Retro Video Game | [`retro-video-game-v4.png`](retro-video-game-v4.png) | Full-frame fictional handheld, system-pink controls, and an LCD game screen made from shipped assets. |
...
The object is a fictional vintage game cartridge, not a console or display. Its molded cream shell fills the square, app-pink accents remain secondary, and the bottom edge exposes one neat row of classic gold connector pins. The large front element is visibly a printed paper label with ink/paper texture rather than glass. The label contains the single title `RetroRapid!`, a rear-view open-wheel racer, and the same four renderer-aligned road paths used by the game. Label text and the central illustration remain safe under aggressive rounded corners.

### Retro Video Game

The fictional handheld fills the square while keeping its screen, system-pink D-pad and buttons, speaker/status slots, and controls visible under rounded masks. The screen resembles RetroRapid! gameplay through its assets and geometry rather than through a title or HUD.
