...
Road masks are generated by `Scripts/GenerateRoadDashMasks.swift` and include `universal`, `watch`, and `tv` idiom variants in each imageset. Lap masks use a checker alpha pattern and are also tinted at runtime to satisfy the same contrast policy as dashed lane markers.
