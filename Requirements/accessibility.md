...
- **Paused grid exploration (universal shared view):** When VoiceOver is running and the scene is paused (user pause, menu/help overlay pause, start/crash pause), left/right gameplay touch regions are hidden from accessibility and the grid is exposed as an accessibility-only exploration overlay. Score/lives remain accessible as read-only status in paused mode.
