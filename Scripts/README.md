...
| `apply-iap-localizations` | Uploads Unlimited Plays IAP localizations (ASC API preferred) | `--check`; `--dry-run`; `--asc-api` (Keychain credentials) |
| `apply-game-center-eu-localizations` | Uploads Game Center achievement + leaderboard localizations via ASC API | `--check`; `--dry-run`; `--achievements-only`; `--leaderboards-only` |
| `print-game-center-eu-localizations` | Prints Game Center achievement copy for manual ASC entry | Read-only |
...
Upload IAP + Game Center localizations (full add-language checklist: [`AppStore/docs/08-locale-expansion.md`](../AppStore/docs/08-locale-expansion.md)):
...
Credentials: Keychain services `RetroRapid ASC Key ID` / `Issuer ID` / `Private Key` (or `ASC_KEY_ID` / `ASC_ISSUER_ID` / `ASC_PRIVATE_KEY`). Prefer `--asc-api` for IAP — Helm CSV upload often no-ops from agent shells. Game Center has no Helm path.
...
Print Game Center achievement copy for manual App Store Connect entry:
