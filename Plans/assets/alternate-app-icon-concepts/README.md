...
These files are the selected art-direction concepts for [the alternate app icon plan](../../alternate_app_icon_plan.md). Theme concepts remain non-runtime references. The app-icon generator copies all three selected Special Editions into their matching `.icon` packages: Retro Cartridge uses neutral-grey v6 Default plus charcoal v6 Dark sources, Retro Video Game uses v4 Default plus v5 Dark, and Retro Game Box uses distinct light-grey/blue/pink and black/red v8 sources.
...
| Retro Cartridge | [`retro-cartridge-v6.png`](retro-cartridge-v6.png) | Full-frame neutral warm-grey cartridge, printed RetroRapid! label, canonical accessibility seal, and gold connector pins. |
...
| Retro Cartridge Dark | [`retro-cartridge-dark-v6.png`](retro-cartridge-dark-v6.png) | Material-aware charcoal cartridge companion with dimensional molding, unchanged pink/gold hierarchy, and the same canonical seal. |
...
| Retro Game Box | [`retro-game-box-v8.png`](retro-game-box-v8.png) | Light-mode handheld-era printing with subtle full-surface paper grain, a clean square perimeter, clearly visible irregular interior scratches, an upright condensed cobalt wordmark, a white/red lozenge, and the canonical accessibility seal. |
| Retro Game Box Dark | [`retro-game-box-dark-v8.png`](retro-game-box-dark-v8.png) | Matte-black boxed-software composition with a left-flush racing-art panel, overlapping title and teal callout, red top tab, smaller inset white/red publisher capsule, left-shifted `BUILT WITH LOVE BY` footer lockup, shorter subtle striped no-neck accessibility figure, authentic interior scratches, and the canonical accessibility seal. |
...
- [`accessibility-up-to-11-seal.png`](accessibility-up-to-11-seal.png) is the canonical transparent medallion composited into both Retro Cartridge and Retro Game Box appearances.
- [`AppStore/assets/brand-marks/`](../../../AppStore/assets/brand-marks/) contains transparent marketing masters derived from the approved title and publisher marks; its seal copy is byte-for-byte identical to this canonical source.
...
- [`special-editions-ios27-mask-dark-preview.png`](special-editions-ios27-mask-dark-preview.png) verifies the current Retro Cartridge v6 and Retro Video Game v5 Dark companions under the template mask, including charcoal material depth, app-pink accents, label/LCD content, navy details, and connector contacts.
...
The label contains the single title `RetroRapid!`, a rear-view open-wheel racer, the same four renderer-aligned road paths used by the game, and the canonical gold certification-style medallion reading `Accessibility up to 11!`. Default and Dark composite the same transparent master at the same scale and position. The seal evokes period game packaging without reproducing Nintendo's name, wordmark, wording, typography, or exact seal geometry. Label text, car, seal, and the central connector contacts remain safe under normal iOS/iPadOS masks; only expendable edge detail may trim under the deliberately aggressive validation crop.
...
- The 2026-09-04 canonical-seal pass generated a standalone original medallion, removed each prior baked seal with localized ImageGen edits, and reconstructed only the small occluded background regions. The same transparent master was then composited deterministically into Retro Cartridge v6 Default/Dark at one shared scale and position and Retro Game Box v8 Default/Dark at one shared smaller scale. This eliminates appearance-to-appearance seal drift and doubled starburst edges while preserving every approved pixel outside the localized replacement regions.
- `./retrorapid assets app-icons` copies the approved neutral-grey v6 Default and charcoal v6 Dark files for Retro Cartridge, the approved v4 Default and v5 Dark files for Retro Video Game, and the approved v8 Default and Dark sources for Retro Game Box instead of procedurally tinting artwork. Each package uses one stable layer with flat Default, Dark, and Tinted image-name specializations; the generator also creates matching adaptive gallery previews. This preserves deterministic runtime output while allowing appearance-specific packaging and material rendering.

### Canonical accessibility seal prompt set

> Newly render a clean standalone version of the original fictional RetroRapid! accessibility seal, closely matching its visual character so it can be reused consistently across several app icons. Use a symmetrical warm-gold many-point starburst medallion with a pale cream circular paper centre, fine dark outer outline, two delicate concentric dark rings, three small dark stars near the top, three near the bottom, and the exact centred wording on three lines: `Accessibility` / `up to` / `11!`. Keep `11!` distinctly larger. Use original late-1980s game-packaging-inspired print character, crisp geometry softened by subtle ink absorption and lightly aged paper/foil texture, a complete frontal silhouette, and generous transparent padding. Preserve genuine alpha and avoid surrounding packaging, backgrounds, extra words, trademarks, watermarks, or imitation of any commercial certification seal's exact geometry, typography, or trade dress.

Seal-free background cleanup used before deterministic compositing:

> Remove only the existing gold `Accessibility up to 11!` seal from the supplied icon. Reconstruct the small occluded label or cardboard region from its immediate surroundings, matching the existing printed texture, colour, lighting, artwork, and geometry. Preserve every other pixel, including all typography, cars, road lines, framing, scratches, controls, and materials. Do not add a replacement seal, placeholder, new text, halo, patch edge, transparency, or composition change.
...
- Selected icon masters remain opaque. The canonical supporting seal intentionally uses true alpha so one master can be composited consistently; no chroma-key processing is used.
