...
- **Challenge modal actions:** Challenge modal always includes a semantic `Done` action; `Other challenges` is available on iOS/macOS where Game Center challenge deep links are supported. On iOS/iPadOS/visionOS, actions are rendered in the same bottom overlay action bar model as game over (ignoring bottom safe area), using the same concentric shape and glass/material treatment for consistency.
