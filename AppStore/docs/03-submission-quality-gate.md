...
Repeatable apply script: `AppStore/scripts/apply_retrorapid_metadata.py`
...
```bash
python3 AppStore/scripts/apply_retrorapid_metadata.py
python3 AppStore/scripts/apply_retrorapid_metadata.py --keywords-only
```

The script applies version-localization fields first, then retries shared App Information name/subtitle once per locale. Use `--keywords-only` when syncing keyword fields without touching promo, description, or What's New.

### Manual App Store Connect fallback

When the API blocks shared App Info (HTTP 409 on name/subtitle), apply fields manually:

1. Run `python3 AppStore/scripts/apply_retrorapid_metadata.py` first so version-localization fields (keywords, promotional text, description, What's New) land on the editable draft.
