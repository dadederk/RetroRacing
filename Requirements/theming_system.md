...
- **Shared road masks** – Theme-agnostic line masks:
  - lane markers: `laneInnerMask`, `laneOuterMask`
  - lap markers: `lapStripMask`
  - all are white alpha masks tinted at runtime

Naming convention: `nameAsset-theme` (e.g. `playersCar-LCD`).

Road masks are generated by `swift run --package-path Scripts generate-road-dash-masks` and include `universal`, `watch`, and `tv` idiom variants in each imageset. Use `--check` for a non-mutating verification. Lap masks use a checker alpha pattern and are also tinted at runtime to satisfy the same contrast policy as dashed lane markers.
