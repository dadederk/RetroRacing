...
- SharePlay countdown audio uses generated F1-style cues: three short ascending beeps mapped to displayed countdown values 3/2/1, followed by one longer final “go” beep when `.inRound` starts. The countdown step scheduler must play each displayed second once, and SharePlay round start must not replay the normal solo start cue.

## Testing Expectations
- Unit tests cover: generated-player completion/volume/stop behavior; generated-player fail-open completion when final player-start readiness fails; recipe modularity (including fail-tail repeat count impact); SharePlay countdown recipe durations/order and once-per-step scheduler behavior; retro tick arpeggio + retro move middle-lane routing; cue-mode routing for tick and move cues; lane move cue style forwarding (including `Haptics` safe/unsafe behavior); lane-cue fail-open grid progression when final player-start readiness fails; paused move input behavior; fail sound + crash haptic on collision; crash fallback fires once when completion is missing; start/resume pauses until sound completion with fallback and restores player visuals immediately after crash; gameplay-session end stops updates, stops audio, and cancels pending crash callbacks; stopAll invoked when game view disappears; volume changes propagate to both `SoundEffectPlayer` and `LaneCuePlayer`; conditional-default/override resolution and migration for audio/speed warning settings.
