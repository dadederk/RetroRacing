...
**Status:** In progress — Pocket/LCD/Cartridge/CRT/Disc layered pilot and v4 Special Edition appearances implemented; Polygon and full appearance/device QA remain open
**Created:** 2026-08-12
...
6. Generate seven opaque RGB `512×512` Default/Dark gallery preview pairs from the same package sources. Default and Dark use separately named universal single-scale image sets; the gallery chooses the asset explicitly from SwiftUI's current color scheme so list-row reuse cannot retain a stale rendition. The high-resolution preview is not treated as an intrinsic `1x` image.
...
- Pocket, LCD, Cartridge, CRT, and Disc now use deterministic semantic SVG road/lane layers plus transparent canonical SpriteKit cars. Their tracked manifests preserve foreground-to-background ordering, full-height projected boundaries, flat Default/Dark/Tinted image source mappings, Mono (`tinted`) treatments, and no flattened `Default.png`. CRT rasterizes its radial vignette into opaque appearance-specific backdrops at the back of World so both supported Composer generations render the falloff, while its frontmost transparent Accents layer retains only the scanline/highlight pattern. Disc keeps its approved circuit exterior as a deterministic transparent World layer behind the road.
...
- `./retrorapid assets app-icons` copies these canonical sources into the Special Edition packages, owns the generated Pocket/LCD/Cartridge/CRT/Disc layers, derives Disc's transparent circuit environment from the approved concept, and generates all seven explicitly named Default/Dark gallery preview pairs. The runtime asset audit verifies package references, geometry, layer order, flat image/opacity appearance specializations, the car/lanes/road Mono hierarchy without freezing manually tuned opacity values, both preview assets, and output drift, while `./retrorapid check` includes the generator's `--check` gate.
