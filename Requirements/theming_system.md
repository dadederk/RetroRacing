...
| Cartridge | Free on iPad/watchOS; Unlimited Plays on iPhone/macOS/tvOS | Vivid home-console-inspired pixel style with a medium grey arcade road, lighter grey exterior field, and yellow lane lines; iPad default. |
| CRT | Free on macOS/watchOS; Unlimited Plays on iPhone/iPad/tvOS | Early-1990s arcade style with the shared grey/yellow perspective road, grass exterior, richer pixel-art sprites, and RGB565 player-red ramp; macOS default. |
| Disc | Free/default on tvOS; free Debug experiment elsewhere | Late-1990s console style with dark asphalt, an electric aqua circuit palette, and a dedicated pre-rendered 32-Bit sprite family. On visionOS it is available in Classic presentation only. |
...
- visionOS defaults to Polygon and always includes it as the free platform theme. It does not show a Polygon Debug toggle because that theme cannot be disabled on visionOS.
- In Debug builds, Settings exposes a Disc toggle on iPhone, iPad, macOS, watchOS, and visionOS, plus a Polygon toggle on iPhone, iPad, macOS, watchOS, and tvOS. Enabling a toggle immediately adds the theme to the selectable catalog as a free QA theme; disabling a currently selected experiment restores the platform default. Builds without Debug UI ignore stored flags and include only the platform-required experimental default.
- visionOS theme selection applies to Classic presentation. Tabletop requires the canonical Polygon model, so the **Play in 3D** ornament is unavailable while Disc is selected.
- Platform-specific presentation can vary, but selection and access rules should remain shared.
- Theme selectors present established shared themes in Pocket, LCD, Cartridge, CRT order even when the platform default is not first, followed by any enabled Disc and Polygon experiments. visionOS exposes only Polygon plus the optional Disc Classic experiment.
