...
- **Shared road masks** – Theme-agnostic lane mark masks: `laneInnerMask`, `laneOuterMask` (white alpha masks tinted at runtime)

Naming convention: `nameAsset-theme` (e.g. `playersCar-LCD`).

Road masks are generated by `Scripts/GenerateRoadDashMasks.swift` and include `universal`, `watch`, and `tv` idiom variants in each imageset.
