...
- iPhone and iPad source copy uses the **recommended seven-slide storyboard** for `en-US`, `en-GB`, `en-AU`, `en-CA`, `es-ES`, and `ca`.
- `en-GB` and `en-AU` use British spelling in overlay copy (`Customise Your Experience`). `en-CA` matches US spelling.
- Mac and Apple Watch source plists include the same locale set; rendered exports still need regeneration from Screenshot Studio.
- Mac Spanish/Catalan screenshot text remains blank and should be filled before export.
