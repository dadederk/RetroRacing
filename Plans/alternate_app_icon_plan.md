...
| Retro Cartridge | Special alternate | ![Retro Cartridge icon concept](assets/alternate-app-icon-concepts/retro-cartridge-v6.png) | Copy the neutral-grey v6 source byte-for-byte into Default and the approved [`retro-cartridge-dark-v6.png`](assets/alternate-app-icon-concepts/retro-cartridge-dark-v6.png) into Dark. Both retain the label, vent, trim, connector accents, and mask-concentric hardware composition; Dark uses genuinely rendered charcoal molding. Both composite the canonical reusable seal at the same scale and position. |
...
| Retro Game Box | Special alternate | ![Retro Game Box icon concept](assets/alternate-app-icon-concepts/retro-game-box-v8.png) | Copy the light-grey/blue/pink v8 cover byte-for-byte into Default and [`retro-game-box-dark-v8.png`](assets/alternate-app-icon-concepts/retro-game-box-dark-v8.png) into Dark. These are deliberately different printings rather than a tint. Both use subtle full-surface cardboard grain, clean uninterrupted square edges, visible irregular interior scratches, and the same canonical seal master used by Retro Cartridge. Light retains the upright cobalt wordmark and white/red lozenge. Dark uses a left-flush art panel, slightly overlapping title and teal tagline box, top red tab, smaller inset publisher capsule, right information rail, left-shifted `BUILT WITH LOVE BY` footer lockup, and a shorter, subtler striped negative-space accessibility figure with friendly rounded limbs and no neck. |
...
Retro Video Game intentionally omits titles, button letters, scores, and other tiny text. Its monochrome beige LCD screen uses the exact shipped iPad LCD player and rival sprites rather than generated interpretations. Its four road boundaries share one gentle projected bend: dash centers follow their respective curves, dash faces stay tangent to them, and length, width, and spacing increase toward the viewer. Retro Cartridge and Retro Game Box are deliberate printed-packaging exceptions to the no-text rule. Retro Cartridge keeps `RetroRapid!` and the canonical `Accessibility up to 11!` certification-style seal inside the central mask-safe region, with one neat row of classic gold contacts below. Retro Game Box keeps `RetroRapid!`, `Race like it is 1985`, an original `A11y up to 11!` publisher mark, and that same canonical seal within the safe region. Cartridge Default and Dark use the master at identical scale and position; Game Box Default and Dark use it at a shared smaller scale appropriate to their denser layouts. Light adds the vertical cobalt wordmark and white/red lozenge; Dark adds the red `ACCESSIBILITY UP TO 11` / `BUILT WITH LOVE BY` footer lockup and horizontally striped negative-space accessibility mark. All period-inspired marks must remain original and must not reproduce Nintendo, Game Boy, Super Nintendo, ESRB, or other third-party names, logos, typography, seal geometry, exact layouts, or trade dress.
...
2. Retro Cartridge keeps the approved neutral-grey v6 PNG byte-for-byte as Default and its approved v6 charcoal-material companion as Dark. Retro Video Game keeps its approved v4 PNG as Default and v5 charcoal-material companion as Dark. Both use the same flat image-name specialization contract on one stable layer. Retro Game Box copies its approved light-grey/blue/pink v8 PNG into Default and its black/red v8 companion into Dark; Tinted maps back to Default. Retro Cartridge and Retro Game Box all derive their medallion from the same canonical transparent seal master. Every Special Edition document canvas explicitly specializes Default, Dark, and Tinted fills. Tinted and Clear are system generated.
...
- Retro Cartridge uses its approved neutral-grey v6 PNG for Default and charcoal-plastic v6 companion for Dark. Retro Video Game uses its approved v4 PNG for Default and charcoal-plastic v5 companion for Dark. Retro Game Box uses its approved light-grey/blue/pink v8 PNG for Default and black/red v8 PNG for Dark. The cartridge and box sources use one canonical `Accessibility up to 11!` seal master. Each package uses one appearance-aware layer. This is a deliberate exception to semantic layering so the approved packaging and material depth remain intact.
