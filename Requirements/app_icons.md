...
- Pocket, LCD, and Cartridge are the layered theme pilot. Their packages place the car first and lane marks second in the foreground Subject group, with the road in the World group behind both. Pocket preserves the approved three-path interrupted-center composition; LCD and Cartridge preserve four broad boundary paths. Every path and both road edges derive from the icon's shared vanishing point, extend from the top to the bottom edge, and grow in width, length, and spacing toward the viewer. The outer paths project beyond the lower corners and the inner paths remain visible around or below the car. Canonical iPad SpriteKit cars remain transparent, nearest-neighbour, opaque, glass-free layers.
...
- CRT, Disc, and Polygon remain flattened during this pilot. Classic is unchanged.
...
- `./retrorapid assets app-icons` deterministically regenerates the Pocket/LCD/Cartridge SVG and car layers, copies both approved Special Edition Default/Dark companion sources into their packages, and generates the five adaptive gallery preview pairs. `--dry-run` reports the mutation plan; `--check` rejects generated-output drift. Icon Composer appearance/material configuration remains in each tracked `icon.json` and is never overwritten by the generator.
