...
| `generate-app-icon-assets` | Generates Pocket/LCD/Cartridge/CRT/Disc Default/Dark semantic Icon Composer layers, rasterizes CRT's opaque vignette backdrops and transparent scanlines, extracts Disc's transparent circuit environment from its approved v4 concept, copies approved Special Edition v4 Default and v5 Dark artwork, and generates seven explicitly named Default/Dark gallery preview pairs as opaque RGB PNGs | `./retrorapid assets app-icons`; `--dry-run` prints generated and obsolete paths; `--check` compares bytes without changing tracked Composer appearance/material configuration |
...
./retrorapid assets spatial --check
./retrorapid assets app-icons --dry-run
./retrorapid assets app-icons --check
