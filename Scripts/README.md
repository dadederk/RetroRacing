...
| `generate-app-icon-assets` | Generates Pocket/LCD/Cartridge/CRT Default/Dark semantic Icon Composer SVG layers, copies approved Special Edition v4 Default and v5 Dark artwork, and generates six adaptive Default/Dark gallery preview pairs | `./retrorapid assets app-icons`; `--dry-run` prints generated and obsolete paths; `--check` compares bytes without changing tracked Composer appearance/material configuration |
