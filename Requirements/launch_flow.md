...
- watchOS keeps its platform navigation. visionOS uses the shared universal-style menu and one shared-session coordinator across the Classic window and a mixed immersive space. Play in 3D acquires transition and placement pause reasons before preflight or surface search; Classic remains available with guidance until the spatial road is anchored and ready for explicit confirmation. Returning, anchor loss, system dismissal, backgrounding, and SharePlay restore the exact run to Classic. Classic gameplay keeps an X in the top-left and Pause/Resume in the top-right; Settings is available from the menu only. See [visionos_gameplay.md](visionos_gameplay.md).

## Play with Friends

- The button is shown only when the composition root supplies `onPlayWithFriendsRequest`.
- It starts a synchronized race on iOS/iPad/macOS/tvOS and visionOS Classic, and is hidden on watchOS. tvOS explains the FaceTime-group prerequisite when direct activation is not eligible. visionOS automatically leaves the spatial immersive space for Classic before applying an incoming match.
